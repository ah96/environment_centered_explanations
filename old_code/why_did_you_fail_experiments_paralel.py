...
def run_single_env_experiment(env_path, planners, explanations, perturbation_types, perturbation_counts, top_k, max_retries):
    ...
    for planner_name in planners:
        ...
        planner_instance.set_environment(env.agent_pos, env.goal_pos, env.grid_size, env.obstacles)
        original_path = planner_instance.plan(return_steps=False)
        path_length = len(original_path[0]) if isinstance(original_path, tuple) and original_path[0] else 0
        ...
        for explainer_name in explanations:
            ...
            original_features, explanation_time = get_important_features(explainer, explainer_name, env, planner_instance, top_k=top_k)
            ...
            for perturbation_type in perturbation_types:
                for num_perturbations in perturbation_counts:
                    for attempt in range(max_retries):
                        ...
                        planner_check.set_environment(perturbed_env.agent_pos, perturbed_env.goal_pos, perturbed_env.grid_size, perturbed_env.obstacles)
                        path_check = planner_check.plan(return_steps=False)
                        path = path_check[0] if isinstance(path_check, tuple) else path_check
                        if not path or len(path) == 0:
                            break
                    else:
                        continue  # skip this perturbation if all retries failed
                    ...
                    perturbed_features, _ = get_important_features(
                        explainer_perturbed, explainer_name, perturbed_env, planner_check, top_k=top_k
                    )
    ...
    return (env_name, results_data, planner_features[env_name])
...
def main():
    ...
    perturbation_types = ["remove", "move"]
    max_retries = 100
    ...
    create_clean_folder("why")

    for env_size in range(10, 21, 2):
        for obstacle_num in range(5, 21, 2):
            OUTPUT = f"why/why_did_you_fail_results_grid_{env_size}_obstacles_{obstacle_num}.csv"
            ...
            print(f"Loaded {len(env_paths)} environments")
            ...
            with Pool(processes=num_workers) as pool:
                for env_name, results_data, pf in tqdm(pool.imap_unordered(run_func, env_paths), total=len(env_paths)):
                    planner_features[env_name] = pf
                    results_all.extend(results_data)

            df = pd.DataFrame(results_all)

            # Compute planner robustness
            for env_name in planner_features:
                for explainer_name in explanations:
                    features = [planner_features[env_name].get(f"{planner}_{explainer_name}") for planner in planners]
                    features = [f for f in features if f is not None]
                    similarities = [jaccard_similarity(f1, f2) for i, f1 in enumerate(features) for f2 in features[i+1:]]
                    robustness = np.mean(similarities) if similarities else 0
                    df.loc[(df["Environment"] == env_name) & (df["Explanation"] == explainer_name), "Planner_Robustness"] = robustness

            if args.average_only:
                avg_df = df.groupby(["Explanation", "Planner", "Num_Perturbations", "Perturbation_Type"]).agg({
